...
\chapter{Théorème des résidus et application au calcul d'intégrales}
...
\begin{theorem}[12.1, p.87]
    Soient $D \subset \Cx$ un ouvert simplement connexe, $\gamma$ une courbe simple fermée régulière (par morceaux) contenue dans $D$ orientée positivement et \linebreak $z_1, z_2, \ldots, z_m \in \inte \gamma$ tels que $z_i \neq z_j$ pour $i \neq j$.
...
\begin{note}
    Cette application est issue de l'\textbf{Application 12.5} page 89 du livre de référence.
\end{note}
...
\begin{note}
    Cette application est issue de l'\textbf{Application 12.7} page 90 du livre de référence.
\end{note}
...
$q(z) = 0 \iff z^4 + 16 = 0 \iff z^4 = -16 = 16 e^{i\pi} \iff z = 2 e^{i\left(\frac{\pi}{4} + \frac{2n\pi}{4}\right)}$ avec $n = 0,1,2,3$.
...
\begin{remark}[1]
    ...
    Lorsque $\alpha \leq 0$, on a $-\alpha r \sin \theta \leq 0$ et $\module{e^{i\alpha z}} < 1$ si $\theta \in [\pi, 2\pi]$ et il faut donc choisir le demi-cercle $C_r$ dans le demi-plan inférieur pour avoir:
